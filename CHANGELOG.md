...
- **Explicit CLI plugins for path scans** — `mix reach.check` accepts repeatable `--plugins` values such as `--plugins Phoenix --plugins Ecto` as an override when automatic target-project inference is not enough.
